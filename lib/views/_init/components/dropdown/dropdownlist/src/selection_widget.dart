...
import 'selection_list_view_props.dart';
...
//! refledtion
    /*  if (widget.showSearchBox && widget.items != null && (!isFistLoad || !widget.isFilteredOnline)) {
      var filterItems = <T>[];
      var items = widget.items;
      _syncItems.clear();
      items = widget.items;
      for (T item in widget.items!) {
        var typerefjson = jsonEncode(item);
        if (widget.textField.isNotEmpty) {
          Map<String, dynamic> map = jsonDecode(typerefjson);
          var fields = widget.textField.split(",");
          for (String field in fields) {
            var data = map[field];
            if (data.toString().toLowerCase().contains(filter.toLowerCase()) || filter.isEmpty) {
              filterItems.add(item);
            }
          }
        }
      }
      _syncItems.addAll(filterItems);
    }*/
