...
class PageMessageDetail extends StatefulWidget {
  final int? id;
  const PageMessageDetail({super.key, required this.id});
  ...
  void connection() {
    socket.on(
        ...
        (data) => {
              ...
              socket.emit('login', {'token': WorkContext().token})
            });
  }

  void sendMessage(types.Message message) {
    socket.emit("private", {'message': jsonEncode(message), 'type': message.type.name, 'to': widget.id});
  }
